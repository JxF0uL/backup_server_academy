...
import pandas as pd
from Dashboard.views import DetalhesProjetosEstacoes
...
# associação de estação
estacaoProjeto = DetalhesProjetosEstacoes.get_object

# associação de projetos e estações
while st.title == estacaoProjeto:
    projetosTotais=+1
    projetosAtivos = filter(['total_em_andamento'], DetalhesProjetosEstacoes)
    projetosStandBy = filter(['stand_by'], DetalhesProjetosEstacoes)
    projetosAtrasados = filter(['atrasado'], DetalhesProjetosEstacoes)

dataframeProjetosStatus = pd.DataFrame({
    "Entregues" : [] ,
    "Em andamento": [projetosAtivos] ,
...
def definir_prioridade(prioridade_projeto):
    if prioridade_projeto == "Alta":
        return "Alta"
    elif prioridade_projeto == "Média":
        return "Média"
    elif prioridade_projeto == "Baixa":
        return "Baixa"
    else:
        return "Sem prioridade definida"
...
def definir_fase(fase):
    if fase == "Andamento":
        ...
        return "Concluido"
    elif fase == "Descontinuado":
        return "Descontinuado"
# irá identificar entre as OPCOES_STATUS a situaçao do projeto
